...
use crate::{f32::math, neon::*, BVec4, BVec4A, Vec2, Vec3, Vec3A, Vec4};
...
#[derive(Clone, Copy)]
#[repr(transparent)]
pub struct Vec4A(pub(crate) float32x4_t);
...
impl Div<Vec4A> for f32 {
    ...
    fn div(self, rhs: Vec4A) -> Vec4A {
        Vec4A(unsafe { vdivq_f32(vld1q_dup_f32(&self), rhs.0) })
    }
}
...
impl Mul<Vec4A> for f32 {
    ...
    fn mul(self, rhs: Vec4A) -> Vec4A {
        Vec4A(unsafe { vmulq_n_f32(rhs.0, self) })
    }
}
...
impl Add<Vec4A> for f32 {
    ...
    fn add(self, rhs: Vec4A) -> Vec4A {
        Vec4A(unsafe { vaddq_f32(vld1q_dup_f32(&self), rhs.0) })
    }
}
...
impl Sub<Vec4A> for f32 {
    ...
    fn sub(self, rhs: Vec4A) -> Vec4A {
        Vec4A(unsafe { vsubq_f32(vld1q_dup_f32(&self), rhs.0) })
    }
}
...
impl From<Vec4A> for float32x4_t {
    #[inline(always)]
...
impl From<float32x4_t> for Vec4A {
    #[inline(always)]
