...
#[cfg(feature = "std")]
pub(crate) use std::simd::StdFloat;

#[cfg(not(feature = "std"))]
pub(crate) use nostd_float::StdFloat;

/// `std::simd::StdFloat` is only available when `std` is, so `no_std` builds using the
/// `core-simd` backend need their own version of it. The methods here are implemented per
/// lane using glam's `math` module, which routes to `libm` when `std` isn't available.
///
/// Only the methods glam actually calls are implemented, so this is deliberately a much
/// smaller trait than the real `StdFloat`.
#[cfg(not(feature = "std"))]
mod nostd_float {
    use crate::f32::math;
    use core::simd::f32x4;

    pub(crate) trait StdFloat: Sized {
        fn sqrt(self) -> Self;
        fn round(self) -> Self;
        fn floor(self) -> Self;
        fn ceil(self) -> Self;
    ...
    impl StdFloat for f32x4 {
        #[inline]
        fn sqrt(self) -> Self {
            Self::from_array(core::array::from_fn(|i| math::sqrt(self[i])))
        }

        #[inline]
        fn round(self) -> Self {
            Self::from_array(core::array::from_fn(|i| math::round(self[i])))
        }

        #[inline]
        fn floor(self) -> Self {
            Self::from_array(core::array::from_fn(|i| math::floor(self[i])))
        }

        #[inline]
        fn ceil(self) -> Self {
            Self::from_array(core::array::from_fn(|i| math::ceil(self[i])))
        }

        #[inline]
        fn trunc(self) -> Self {
            Self::from_array(core::array::from_fn(|i| math::trunc(self[i])))
        }

        #[inline]
        fn mul_add(self, a: Self, b: Self) -> Self {
            Self::from_array(core::array::from_fn(|i| math::mul_add(self[i], a[i], b[i])))
        }
    }
}
