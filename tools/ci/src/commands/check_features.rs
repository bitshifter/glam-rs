use argh::FromArgs;
use xshell::{cmd, Shell};

use crate::args::Args;
use crate::prepare::{Prepare, PreparedCommand};

#[derive(FromArgs, Default)]
#[argh(subcommand, name = "check-features")]
/// Check compilation of feature combinations. Without --index, checks all sets.
pub struct CheckFeatures {
    #[argh(option, description = "check a single feature set by 1-based index")]
    index: Option<usize>,

    #[argh(switch, description = "list available feature sets and exit")]
    list: bool,
}

impl Prepare for CheckFeatures {
    fn prepare<'a>(&self, sh: &'a Shell, _args: &Args) -> Vec<PreparedCommand<'a>> {
        if self.list {
            crate::features::print_feature_sets();
            return Vec::new();
        }

        let mut cmds = Vec::new();

        let sets = crate::features::resolve_sets(self.index);
        let total = crate::features::FEATURE_SETS.len();

        for (i, features) in sets.iter().enumerate() {
            let idx = self.index.unwrap_or(i + 1);
            let cmd = cmd!(
                sh,
                "cargo check --tests --no-default-features --features {features}"
            );
            cmds.push(PreparedCommand {
                name: format!("check [{idx}/{total}]: {features}"),
                command: cmd,
                failure_message: "check feature set failed",
            });
        }

        let cmd = cmd!(sh, "cargo check").env("RUSTFLAGS", "-C target-feature=+fma");
        cmds.push(PreparedCommand {
            name: "check FMA".into(),
            command: cmd,
            failure_message: "FMA check failed",
        });

        let cmd = cmd!(sh, "cargo check -p glam-no_std");
        cmds.push(PreparedCommand {
            name: "check glam-no_std".into(),
            command: cmd,
            failure_message: "no_std check failed",
        });

        cmds
    }
}
