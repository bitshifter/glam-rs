macro_rules! deps {
    () => {
        "arbitrary approx-05 bytemuck encase encase-013 float_eq mint rand-010 rkyv-08 bytecheck serde speedy zerocopy debug-glam-assert"
    };
}
...
pub(crate) const FEATURE_SETS: &[&str] = &[
    ...
    "std all-types rkyv bytecheck rand approx",
];

// MSRV reduced set — some optional deps need a newer rustc
pub(crate) const MSRV_FEATURES: &str =
    "all-types arbitrary approx-05 mint speedy debug-glam-assert";
...
pub(crate) const CORE_SIMD_FEATURES: &str =
    "core-simd arbitrary approx-05 bytemuck encase encase-013 float_eq mint rand-010 rkyv-08 bytecheck serde speedy debug-glam-assert";
