...
fn write_test_vec3(out: &mut impl Write, name: &str) -> std::io::Result<()> {
    const SIZE: usize = 3;
    const E: [char; SIZE] = ['x', 'y', 'z']; // element name
        ...
        for e1 in 0..SIZE {
            for e2 in 0..SIZE {
                ...
                writeln!(
                    out,
                    "    assert_eq!(v.{}{}{}(), {}({}, {}, {}));",
                    E[e0], E[e1], E[e2], name, V[e0], V[e1], V[e2]
                )?;
            }
        }
    }
...
fn write_test_vec2(out: &mut impl Write) -> std::io::Result<()> {
    const SIZE: usize = 2;
    const E: [char; SIZE] = ['x', 'y']; // element name
    const V: [&str; SIZE] = ["1.0", "2.0"]; //element value
    write!(
        out,
        r#"
#[test]
